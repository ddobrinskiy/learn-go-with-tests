...
func TestRacer(t *testing.T) {
	...
	t.Run("test normal case", func(t *testing.T) {
		...
		want := fastURL
		got, err := ConfigurableRacer(slowURL, fastURL, 100*time.Millisecond)
	...
	t.Run("returns an error if a server doesn't respond within timeout", func(t *testing.T) {
		server := makeDelayedServer(200 * time.Millisecond)
		defer server.Close()

		_, err := ConfigurableRacer(server.URL, server.URL, 100*time.Millisecond)
